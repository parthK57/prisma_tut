...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (resp?.status === 200) alert("Login successfull!");
    } catch (error) {
      if (error?.response?.status === 401) alert(error?.response?.data?.res);
  ...
  return (
    <div className="w-screen h-screen flex justify-center items-center ">
      <div className="bg-blue-200 w-72 flex flex-col items-center shadow-md rounded-lg px-5 py-5">
        <span className="font-medium text-2xl border-b py-1 border-black w-full text-center">Login</span>
        <form
          action=""
